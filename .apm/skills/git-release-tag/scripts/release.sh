...
Options:
  ...
  --ticket TICKET   JIRA ticket ID to include in commit message (e.g., PROJ-123)
...
Examples:
  ...
  bash scripts/release.sh minor --ticket PROJ-123   # Include JIRA ticket in commit
USAGE
  exit 0
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ticket)
      shift
      [[ -z "${1:-}" ]] && fail "--ticket requires a JIRA ticket ID (e.g., PROJ-123)" 1
      TICKET="$1"
      ;;
