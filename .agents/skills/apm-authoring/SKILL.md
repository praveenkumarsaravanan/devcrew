...
| Event | Fires when | Cursor | Codex |
|---|---|---|---|
| `afterFileEdit` | A file matching the pattern is edited | Yes (prompt) | No¹ |
| `preToolUse` | Before a tool is executed (can block the action) | Yes (prompt) | No¹ |

¹ Codex uses PascalCase command hooks (`PreToolUse`) with `type: "command"`, not prompt hooks. DevCrew deploys Codex hooks via `scripts/setup-codex-hooks.sh` after `apm install --target codex`. Run `apm run postinstall` to apply both Cursor prompts and Codex hooks.

### Writing Guidelines

- Each hook must have a `name`, `type`, and `prompt` (Cursor/Claude prompt hooks).
