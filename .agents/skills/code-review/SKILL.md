...
description: >
  ...
  evaluate code changes. Covers Go, Python, TypeScript/Node, Java, data feeds,
  mappings, interoperability contracts, regulated data, and frontend frameworks
...
- Determine the languages and frameworks involved (Go, Python, server-side TypeScript/Node, Java, data ingestion/mapping, interoperability contracts, regulated data, React, Vue, Angular, Svelte) to apply the correct checks.
...
- **TypeScript/Node**: Check that async functions have proper try/catch or `.catch()`. Ensure error types are narrowed, not caught as `any`. Verify rejected promises cannot escape API, worker, job, or CLI boundaries.
...
- Verify runtime validation at untrusted boundaries: HTTP body, params, query, headers, env config, queue payloads, webhooks, and external API responses.
- Verify `process.env` is read through typed config, not scattered through the codebase.
- Verify outbound HTTP/SDK/database/cache/queue calls have explicit timeout behavior.
- Verify logs redact tokens, cookies, authorization headers, secrets, raw payloads, and PII.
- For workers, scheduled jobs, and event handlers, verify idempotency, retry limits, poison message handling, and dead-letter behavior.

**Data platform:**
- Verify source contract, schema validation, landing/audit trail, reject/quarantine handling, idempotency, retry/DLQ behavior, reconciliation, replay/backfill safety, and operational runbook coverage.
- Verify mapping changes include golden-file tests and data quality reporting.
- Confirm raw sensitive payloads are not logged, exposed in reports, or embedded in fixtures.

**Interoperability contracts:**
- Verify OpenAPI, AsyncAPI, webhook/event, protobuf, GraphQL, SDK, partner API, and file/feed changes preserve backward compatibility or include versioning, deprecation, and consumer migration.
- Verify provider, consumer, compatibility, webhook/event, or file/feed contract tests cover the changed boundary.
- Verify representative examples exist for normal, invalid, edge, and historical cases.

**Regulated data:**
- Verify sensitive fields are classified, minimized, access-controlled, redacted, audited, and covered by retention/deletion expectations.
- Verify tests, fixtures, examples, docs, prompts, dashboards, reports, and runbooks use synthetic or approved de-identified data.
- Verify sensitive reads, exports, deletions, permission changes, replay/backfill, and bulk access emit audit events without raw sensitive values.
...
- **TypeScript/Node:** Verify tests cover runtime validation, async error paths, timeout/retry behavior, sensitive logging, and worker/job idempotency when those areas are touched.
- **Data platform:** Verify golden-file mapping tests, schema drift cases, reject/quarantine reason codes, idempotency, replay/backfill dry-run, and reconciliation checks are covered.
- **Interoperability contracts:** Verify provider/consumer/compatibility tests for changed OpenAPI, AsyncAPI, webhook/event, protobuf, GraphQL, SDK, or file/feed contracts.
- **Regulated data:** Verify redaction/masking, synthetic/de-identified fixtures, authorization, audit events, and retention/deletion behavior are tested.
...
- **`typescript-node-standards`** — Detailed review checklist for TypeScript/Node backend services, workers, and jobs.
- **`data-ingestion`** — Detailed review checklist for data feeds, validation, replay, reconciliation, and runbooks.
- **`interoperability-contracts`** — Detailed review checklist for external and cross-team contract compatibility.
- **`regulated-data-handling`** — Detailed review checklist for sensitive data classification, redaction, synthetic data, audit, and retention.
