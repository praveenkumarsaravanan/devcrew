...
name: api-design
description: >
  REST, gRPC, and API contract design guidance for backend services. Use when
  designing new endpoints, reviewing API contracts, creating OpenAPI specs, or
...
- Needs to coordinate an AsyncAPI, webhook, event, protobuf, or external contract that also requires compatibility review
...
- Is building or reviewing **FHIR** resources, profiles, or clinical REST APIs — activate `fhir-health-interop` (FHIR rules take precedence over generic REST guidance)
...
- If the API is public, partner-facing, cross-team, event-driven, webhook-based, or contract-sensitive, activate `interoperability-contracts` for compatibility, consumer migration, and contract-test guidance.
...
### 11. Review Contract And Data Sensitivity

- For OpenAPI, AsyncAPI, webhook, event, gRPC/protobuf, GraphQL, file/feed, SDK, public, partner, or cross-team contracts, activate `interoperability-contracts`.
- Treat removed fields, changed types, changed requiredness, changed auth, changed error semantics, or changed retry behavior as breaking unless proven otherwise.
- Require provider/consumer/compatibility contract tests for changed external or cross-team contracts.
- If the API carries sensitive fields, activate `regulated-data-handling` and document classification, redaction, synthetic examples, audit events, and retention expectations.
...
- **Preserve contract compatibility.** Breaking external or cross-team API changes require versioning, deprecation, consumer migration, and contract tests.
- **Classify sensitive fields.** API contracts carrying sensitive data must include redaction, synthetic examples, audit, and retention expectations.
...
- **`interoperability-contracts`** — Use for external, partner, cross-team, AsyncAPI, webhook, event, file/feed, protobuf, GraphQL, or SDK contract compatibility.
- **`regulated-data-handling`** — Use when API contracts carry sensitive or regulated fields.
