...
Options:
  ...
  --ticket TICKET   ticket ID to include in commit message (e.g., ISSUE-123)
...
Examples:
  ...
  bash scripts/release.sh minor --ticket ISSUE-123   # Include ticket in commit
USAGE
  exit 0
}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ticket)
      shift
      [[ -z "${1:-}" ]] && fail "--ticket requires a ticket ID (e.g., ISSUE-123)" 1
      TICKET="$1"
      ;;
