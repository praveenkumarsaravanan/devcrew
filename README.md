...
Organization-wide agent package distributing engineering skills, agents, rules, and MCP server configs across Cursor, GitHub Copilot, and Claude Code via [Microsoft APM](https://microsoft.github.io/apm/).
...
> **Note:** Claude Code support requires a workaround script (`apm run deploy-claude`) until [microsoft/apm#751](https://github.com/microsoft/apm/issues/751) is resolved. See [Claude Code Setup](#claude-code-setup) below.
...
The `.apm/` directory holds **source primitives**. Your IDE reads from compiled output directories (`.cursor/skills/`, `.github/instructions/`, `.claude/skills/`, etc.). Without this step, the agent has no access to the skills defined in this repo. Re-run `apm compile` after adding or modifying any primitive.

4. **Claude Code users** — deploy primitives to `.claude/`:

```sh
apm run deploy-claude
```

This is a workaround for [microsoft/apm#751](https://github.com/microsoft/apm/issues/751). Re-run after adding or modifying any primitive.
...
**Claude Code** — APM does not yet deploy to `~/.claude/` automatically ([#751](https://github.com/microsoft/apm/issues/751)). After global install, run the deploy script from the package directory:

```sh
bash ~/.apm/packages/engineering-agent-platform/scripts/deploy-claude.sh
```
...
**Claude Code** — after `apm install`, also run:
...
Everything else APM generates (`.cursor/`, `.claude/`, `.github/`, `AGENTS.md`, `CLAUDE.md`, `.mcp.json`) should be **gitignored** — each developer regenerates them locally with `apm install`.
...
~/.claude/                                 # Claude Code (via deploy-claude.sh)
...
your-repo/
  ...
  CLAUDE.md                                # Compiled context (gitignored)
  .cursor/                                 # Cursor (gitignored)
    rules/*.mdc
    agents/*.md
    skills/{name}/
    hooks.json
  .claude/                                 # Claude Code (gitignored, via deploy-claude.sh)
    rules/*.md
    agents/*.md
    skills/{name}/
    commands/*.md
